...
public class ButtonNormal extends ButtonFeature {
    ...
    private int redColor = new Color(255, 10, 10).getRGB();
    ...
    public void drawButton(Minecraft mc, int mouseX, int mouseY) {
        if (visible) {
            ...
            if (main.getConfigValues().isRemoteDisabled(feature)) {
                fontColor = new Color(255, 60, 60).getRGB();
            }
            ...
            if (creditFeature != null && !main.getConfigValues().isRemoteDisabled(feature)) {
                scale = 0.8F;
                GlStateManager.pushMatrix();
                GlStateManager.scale(scale, scale, 1);
            ...
            if (main.getConfigValues().isRemoteDisabled(feature)) {
                MinecraftReflection.FontRenderer.drawCenteredString(Message.MESSAGE_FEATURE_DISABLED.getMessage(), textX, textY + 25, redColor);
            }
        }
    }
